...
export function ViewMembersAccordion() {
  ...
  useEffect(() => {
    (async () => {
      let users = await (
        await fetch(`http://backend-saloon.onrender.com/admin/getUsers`)
      ).json();
      setMemberList(users);
    })();
  }, []);
  ...
  const handleSave = (index, member) => {
    ...
    (async () => {
      ...
      let response = await fetch(`http://backend-saloon.onrender.com/admin/updateUser`, {
        method: "PUT",
  ...
  const handleDelete = (index, id) => {
    (async () => {
      const packet = { id };
      let response = await fetch(`http://backend-saloon.onrender.com/admin/removeUser`, {
        method: "DELETE",
        body: JSON.stringify(packet),
        headers: {
          "Content-Type": "application/json",
