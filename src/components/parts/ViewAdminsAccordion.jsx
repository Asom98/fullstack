...
export function ViewAdminsAccordion() {
  ...
  useEffect(() => {
    (async () => {
      let admins = await (
        await fetch(`http://backend-saloon.onrender.com/admin/getAdmins`)
      ).json();
      setAdminList(admins);
    })();
  }, []);
  ...
  const handleSave = (index, admin) => {
    ...
    (async () => {
      ...
      let response = await fetch(`http://backend-saloon.onrender.com/admin/updateAdmin`, {
        method: "PUT",
  ...
  const handleDelete = (id) => {
    (async () => {
      const packet = { id };
      let response = await fetch(`http://backend-saloon.onrender.com/admin/removeAdmin`, {
        method: "DELETE",
        body: JSON.stringify(packet),
        headers: {
          "Content-Type": "application/json",
